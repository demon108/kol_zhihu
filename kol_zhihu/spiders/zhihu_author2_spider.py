...
#         return []
    ...
    def move_url_from_mongo_redis(self):
        conn = db.connect('192.168.241.25', 'zhihu')
        authorids = db.find(conn, 'author2',{},{'id':'true'})
...
#             self.reqs_pool.append(answer_req)
    ...
    def start_requests(self):
        print 'request pool: ',self.get_spider_pending_cnt(0)
        reqs = self.make_requests()
        return reqs
        
#         if len(self.reqs_pool)>5000:
#             cur_reqs = self.reqs_pool[:5000]
#             self.reqs_pool = self.reqs_pool[5000:]
#             self.total = 5000
#             return cur_reqs
...
#         return self.reqs
    ...
    def parse_author_info(self,response):
        ...
        if not redis.check_set_value(self.redis_conn, self.redis_key_list, zl_url):
            redis.add_set_value(self.redis_conn, self.redis_key_list, zl_url)
            yield Request(zl_url,callback=self.parse_author_follow_zl,headers={'User-Agent':user_agent},cookies=cookie,meta={'author_id':author_id})
        if int(follow_zl_num)>20:
            ...
            method = 'POST'
            yield FormRequest(url,method=method,formdata=formdata,headers={'User-Agent':user_agent},meta={'author_id':author_id},callback=self.parse_author_follow_zl2)
        #作者关注的话题
        topic_url = 'http://www.zhihu.com/people/%s/topics'%(author_id)
        yield Request(topic_url,callback=self.parse_author_follow_topic,headers={'User-Agent':user_agent},cookies=cookie,meta={'author_id':author_id})
        if int(follow_topic_num)>20:
            ...
            for i in range(int(num)):
                formdata = {'start':str(0),'offset':str((i+1)*20),'_xsrf':'%s'%(xsrf)}
                yield FormRequest(topic_url,method=method,formdata=formdata,headers={'User-Agent':user_agent},cookies=cookie,meta={'author_id':author_id},callback=self.parse_author_follow_topic2)
...
#         for link in links:
    ...
    def parse_author_follow_zl2(self,response):
        ...
        for data in datas:
            ...
            zl_url = sel.xpath('//a[@class="zm-list-avatar-link"]/@href').extract()[0]
    ...
    def parse_author_follow_topic(self,response):
        ...
        try:
            ...
            content = response.body
        sel = Selector(text=content)
        tmp_topics = sel.xpath('//div[@class="zm-profile-section-main"]')
        for tmp_topic in tmp_topics:
            topic_url = tmp_topic.xpath('a/@href').extract()[1]
    ...
    def parse_author_ask(self,response):
        ...
        for next_page in next_pages:
            ...
            if not self.next_ask_page_re.search(next_url):
                continue
            if redis.check_set_value(self.redis_conn, self.redis_key_unfetch, next_url):
                continue
            redis.add_set_value(self.redis_conn, self.redis_key_unfetch, next_url)
