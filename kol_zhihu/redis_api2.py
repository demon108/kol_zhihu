...
def add_url(handle,url,key = 'zhihu_urls',score = 0):
    score = random.random()
    handle.zadd(key,score,url)

def delete_url(handle,url,key = 'zhihu_urls'):
    handle.zrem(key,url)

def get_urls(handle,num,key = 'zhihu_urls'):
    urls = handle.zrevrange(key,0,num)
    for url in urls:
